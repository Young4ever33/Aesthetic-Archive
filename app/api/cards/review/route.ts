...
const transitions: Record<string, string[]> = { private: ['pending'], pending: ['rejected', 'approved'], rejected: ['pending'], approved: ['published', 'unpublished'], published: ['unpublished'], unpublished: ['pending'] };
...
export async function POST(request: Request) {
  ...
  try {
    ...
    const isAdmin = await supabase.from('profiles').select('role').eq('id', user.id).single().then(({ data }) => data?.role === 'admin');
    const target = body.action === 'submit' ? 'pending' : body.action === 'approve' ? 'approved' : body.action === 'reject' ? 'rejected' : body.action === 'publish' ? 'published' : 'unpublished';
    ...
    if (body.action !== 'submit' && !isAdmin) return out(requestId, 403, { code: 'FORBIDDEN', message: 'Only reviewers can perform this action' });
    ...
    const { data: updated, error } = await client.from('aesthetic_cards').update({ publish_status: target, visibility: target === 'published' ? 'public' : card.visibility, reviewed_at: isAdmin ? new Date().toISOString() : null }).eq('id', body.cardId).select('id, publish_status, visibility, reviewed_at').single();
    if (error) return out(requestId, 500, { code: 'REVIEW_UPDATE_FAILED', message: 'Unable to update review status' });
    await client.from('publish_reviews').insert({ card_id: body.cardId, owner_id: card.owner_id, reviewer_id: isAdmin ? user.id : null, status: target, note: body.note || null, reviewed_at: isAdmin ? new Date().toISOString() : null });
