...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (imageModels.length === 0 && textModels.length === 0) return errorResponse(id, 400, 'INVALID_REQUEST', 'Add at least one model');
    ...
    const { data, error } = await admin.from('ai_providers').insert({
      owner_id: user.id,
      ...providerFields({ ...body, imageModels, textModels }),
      name,
      type,
