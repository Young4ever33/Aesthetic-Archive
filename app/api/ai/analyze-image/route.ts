...
const prompt = (body: ReturnType<typeof validateAnalyzeImageRequest>) => `Analyze the reference image for an aesthetic archive. Return ONLY valid JSON with these keys: category, title, titleZh, summary, visibleFacts (array), culturalContext (array; mark uncertainty), inferences (array), materials (array), lighting (array), geometry (array), typography (array), palette (array of hex strings), composition, useCases (array), promptZh, promptEn, negativePromptZh, negativePromptEn, confidence (0 to 1), reviewNotes (array), source, rightsStatus. Distinguish visible facts from cultural interpretation. Never invent a designer, architect, public figure, brand, location, date, provenance, software, or hidden material. promptZh and promptEn must describe the same single representative scene and each must be complete and directly runnable. Lock a concrete subject, quantity and relation, foreground/midground/background, one camera position and lens character, light direction and color temperature, material surface and joints, dominant/support/accent color proportions, and output texture. State likely model-drift boundaries in the positive Prompt as well as the negative Prompt. Do not use placeholders, bracketed fill-in fields, variable lists, "user-provided subject", "replace with", use-case metadata, research notes, quality slogans, or model parameters. promptZh and negativePromptZh must contain Chinese instructions only; promptEn and negativePromptEn must contain English only. Keep both negative Prompts separate and specific to structural, anatomical, typographic, palette, material, identity, text, logo, and watermark failures relevant to this image. Topic: ${body.topic || 'not provided'}. Project context: ${body.projectContext || 'not provided'}.`;
