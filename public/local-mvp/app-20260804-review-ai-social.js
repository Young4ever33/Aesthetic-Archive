...
function openDetail(item) {
  ...
  els.detailCultural.textContent = culturalBackground || (isEnglish() ? 'Cultural origin requires verification.' : '文化来源待考证。');
...
function normalizeProviderDraft(data) {
  ...
  return {
    ...
    cultural: uniqueGeneratedPhrases(data.culturalContext || data.cultural_context || data.cultural || data.culturalBackground || data.historicalOrigin || '', 2).join(' / '),
    elements: uniqueGeneratedPhrases(data.designElements || data.elements || data.visibleFacts || '', 6).join(' / '),
    ...
    composition: uniqueGeneratedPhrases(data.composition || '', 1).join(''),
    useCases: uniqueGeneratedPhrases(data.useCases || data.use_cases || '', 5).join(', '),
...
async function analyzeArchiveImage() {
  ...
  try {
    providerDraft = useProvider ? await analyzeImageWithProvider(imageList, settings) : null;
  } catch (error) {
    if (els.archiveAiStatus) els.archiveAiStatus.textContent = `Provider 调用失败：${error.message}`;
