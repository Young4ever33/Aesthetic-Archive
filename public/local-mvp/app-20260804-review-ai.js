...
function applyWorkspaceTranslations(lang) {
  ...
  set('#collage-summary span', en ? 'BOARD SUMMARY' : '画板总结'); set('#collage-summary p', en ? 'Add references from the plaza, archive, or detail view.' : '从广场、个人审美库或详情页把参考图加入项目风格板。'); set('#collage-export-menu summary', en ? 'Export' : '导出'); set('#clear-collage-btn', en ? 'Clear Board' : '清空画板'); set('#collage-inspector', en ? 'Select an element to edit, duplicate, reorder or remove it.' : '选择一个元素后，可以编辑、复制、调整层级或移除。'); set('#detail-add-gallery', en ? 'Add image set to board' : '将整组图片加入画板'); set('#detail-background-title', en ? 'Aesthetic Background' : '风格背景'); set('#detail-elements-block h3', en ? 'Design Elements' : '设计要素'); set('#detail-composition-block h3', en ? 'Composition' : '构图方式'); set('#detail-palette-block h3', en ? 'Palette' : '色卡'); set('#detail-use-cases-block h3', en ? 'Use Cases' : '使用场景'); set('#detail-prompt-zh-block h3', en ? 'Chinese Prompt' : '中文提示词'); set('#detail-prompt-en-block h3', en ? 'English Prompt' : '英文提示词'); set('#detail-negative-block h3', en ? 'Negative Prompt' : '负面提示词'); set('#profile-status', en ? 'Profile is stored in this browser.' : '资料保存在当前浏览器。'); set('#template-status', en ? 'System default has not been edited.' : '未修改系统默认模板。'); set('#setting-template-test-result', en ? 'Save a template, then check whether its Prompt is specific and reusable.' : '保存模板后，可以在这里检查 Prompt 是否具体、可复用。'); set('#setting-template-state', en ? 'System default / Read only' : '系统默认 / Read only');
}
