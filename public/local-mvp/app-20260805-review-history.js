...
async function syncCloudWorkspace() {
  cloudState = 'online';
  const [cardsResult, publicResult, savedResult] = await Promise.allSettled([
  ...
  const responses = [cardsResult, publicResult, savedResult]
  ...
  if (cardsResult.status === 'fulfilled' && cardsResult.value.ok) {
    const payload = await cardsResult.value.json().catch(() => ({}));
    cloudCards = (payload.data || []).map(cloudCardToLocal);
  }
  if (publicResult.status === 'fulfilled' && publicResult.value.ok) {
    const payload = await publicResult.value.json().catch(() => ({}));
    cloudPublicCards = (payload.data || []).map(cloudCardToLocal);
  }
...
function openDetail(item) {
  ...
  const culturalBackground = item.historicalOrigin || item.culturalBackground || '';
  els.detailElements.textContent = uniqueGeneratedPhrases([
    item.designElements || item.designLogic,
    item.typography ? `${isEnglish() ? 'Typography' : '字体'}：${item.typography}` : '',
    item.imagery ? `${isEnglish() ? 'Imagery' : '图像'}：${item.imagery}` : ''
  ].filter(Boolean), 6).join(' / ') || ui('elementsPending');
  ...
  renderGallery(0);
  els.galleryNote.textContent = culturalBackground || (isEnglish() ? 'Cultural origin requires verification.' : '文化来源待考证。');
...
function renderDetailPalette(item) {
  const colors = [...new Set((item.palette || []).map(color => (typeof color === 'string' ? color : color.hex || '').toUpperCase()).filter(validHex))].slice(0, 6);
  els.detailPalette.innerHTML = colors.map(hex => `<div class="detail-color" style="background:${escapeHTML(hex)}"><span>${escapeHTML(hex)}</span></div>`).join('');
}
...
let reviewQueue = [];
let reviewHistory = [];
let reviewView = 'queue';
let confirmedReviewUserId = '';
let serverReviewAccess = false;
...
function renderReviewHistory() {
  ...
  if (!reviewHistory.length) {
    els.reviewGrid.innerHTML = `<div class="empty-state"><span>${en ? 'NO HISTORY' : 'NO HISTORY'}</span><h3>${en ? 'No reviewed cards yet.' : '暂无审核记录。'}</h3><p>${en ? 'Approved and rejected cards will appear here.' : '已通过和已驳回的卡片会显示在这里。'}</p></div>`;
    return;
  }
  els.reviewGrid.innerHTML = reviewHistory.map(entry => {
    const card = entry.card || {};
    const image = reviewCardImage(entry);
    const approved = entry.status === 'published';
    const statusLabel = approved ? (en ? 'Approved' : '已通过') : (en ? 'Rejected' : '已驳回');
    const time = entry.reviewed_at ? new Date(entry.reviewed_at).toLocaleString(en ? 'en-US' : 'zh-CN', { hour12: false }) : '';
    return `<article class="review-card review-card-history" data-review-card="${escapeHTML(entry.card_id || '')}">
      <div class="review-card-image">${image ? `<img src="${escapeHTML(image)}" alt="${escapeHTML(card.title || '')}">` : '<span>No image</span>'}</div>
      <div class="review-card-body"><div class="card-meta"><span>${escapeHTML(card.category || 'Other')}</span><span class="review-status-badge ${approved ? 'is-approved' : 'is-rejected'}">${statusLabel}</span></div>
      <h3>${escapeHTML(card.title_zh || card.title || 'Untitled')}</h3><p class="review-card-en">${escapeHTML(card.title || '')}</p>
      <div class="review-review-notes"><strong>${en ? 'Review note' : '审核备注'}</strong><span>${escapeHTML(entry.note || '暂无')}</span></div>
      <div class="review-history-meta"><span>${en ? 'Reviewed at' : '审核时间'}：${escapeHTML(time)}</span></div></div>
    </article>`;
  }).join('');
}

function setReviewView(view) {
  reviewView = view === 'history' ? 'history' : 'queue';
  document.querySelectorAll('[data-review-view]').forEach(button => {
    const active = button.dataset.reviewView === reviewView;
...
async function loadReviewQueue() {
  ...
  if (reviewView === 'history') return loadReviewHistory();
...
async function loadReviewHistory() {
  if (!els.reviewGrid) return;
  const en = isEnglish();
  if (els.reviewStatus) els.reviewStatus.textContent = en ? 'Loading review history…' : '正在读取审核记录…';
  try {
    const response = await fetch('/api/admin/reviews?view=history', { credentials: 'same-origin' });
    const payload = await response.json().catch(() => ({}));
    if (response.status === 403 || response.status === 401) {
      confirmedReviewUserId = '';
      serverReviewAccess = false;
      ...
      if (user) setUser({ ...user, role: 'user' });
      updateReviewAccess();
      toast(en ? 'Review access is not assigned to this account.' : '当前账号未分配审核权限。');
      return;
    }
    if (!response.ok) throw new Error([payload.error?.message || `Review API ${response.status}`, payload.error?.code, payload.error?.stage, payload.error?.databaseCode, payload.requestId].filter(Boolean).join(' · '));
    reviewHistory = Array.isArray(payload.data) ? payload.data : [];
    if (els.reviewStatus) els.reviewStatus.textContent = `${reviewHistory.length} ${en ? 'reviewed cards' : '条审核记录'}`;
    renderReviewHistory();
  } catch (error) {
    if (els.reviewStatus) els.reviewStatus.textContent = `${en ? 'Unable to load history: ' : '审核记录加载失败：'}${error.message}`;
    els.reviewGrid.innerHTML = `<div class="empty-state"><span>${en ? 'HISTORY UNAVAILABLE' : '审核记录不可用'}</span><h3>${en ? 'The review service is unavailable.' : '审核服务暂时不可用。'}</h3><p>${en ? 'Check Supabase authentication and try refreshing.' : '请检查 Supabase 登录状态后刷新。'}</p></div>`;
  }
}

async function reviewCard(cardId, action) {
  const buttons = [...document.querySelectorAll(`[data-review-id="${CSS.escape(cardId)}"]`)];
  ...
  try {
    ...
    toast(`${isEnglish() ? 'Review failed: ' : '审核失败：'}${error.message}`);
  }
}
...
function applyWorkspaceTranslations(lang) {
  ...
  set('#collage-summary span', en ? 'BOARD SUMMARY' : '画板总结'); set('#collage-summary p', en ? 'Add references from the plaza, archive, or detail view.' : '从广场、个人审美库或详情页把参考图加入项目风格板。'); set('#collage-export-menu summary', en ? 'Export' : '导出'); set('#clear-collage-btn', en ? 'Clear Board' : '清空画板'); set('#collage-inspector', en ? 'Select an element to edit, duplicate, reorder or remove it.' : '选择一个元素后，可以编辑、复制、调整层级或移除。'); set('#detail-add-gallery', en ? 'Add image set to board' : '将整组图片加入画板'); set('#detail-background-title', en ? 'Aesthetic Background' : '风格背景'); set('#detail-elements-block h3', en ? 'Design Elements' : '设计要素'); set('#detail-composition-block h3', en ? 'Composition' : '构图方式'); set('#detail-palette-block h3', en ? 'Palette' : '色卡'); set('#detail-use-cases-block h3', en ? 'Use Cases' : '使用场景'); set('#detail-prompt-zh-block h3', en ? 'Chinese Prompt' : '中文提示词'); set('#detail-prompt-en-block h3', en ? 'English Prompt' : '英文提示词'); set('#profile-status', en ? 'Profile is stored in this browser.' : '资料保存在当前浏览器。'); set('#template-status', en ? 'System default has not been edited.' : '未修改系统默认模板。'); set('#setting-template-test-result', en ? 'Save a template, then check whether its Prompt is specific and reusable.' : '保存模板后，可以在这里检查 Prompt 是否具体、可复用。'); set('#setting-template-state', en ? 'System default / Read only' : '系统默认 / Read only');
}
...
function applyTranslations(lang) {
  ...
  text('#archive-ai-status', lang === 'en' ? 'Original images remain in this form until you save. Configure an AI service to analyze all references.' : '原图在保存前仅载入当前表单。请配置AI服务后分析全部参考图。');
...
function uniqueGeneratedPhrases(value, limit = 6) {
  const values = Array.isArray(value) ? value : String(value || '').split(/[\/｜|；;。\n]+/);
  const seen = new Set();
  return values.map(item => String(item).trim().replace(/^[\d.)、\s-]+/, '')).filter(item => {
    const key = item.toLowerCase().replace(/[\s，,、:：_-]+/g, '');
...
function normalizeProviderDraft(data) {
  ...
  const used = new Set();
  const distinctField = (value, limit, separator) => uniqueGeneratedPhrases(value, limit).filter(item => {
    const key = item.toLowerCase().replace(/[\s，,、:：_-]+/g, '');
    if ([...used].some(previous => previous === key || previous.includes(key) || key.includes(previous))) return false;
    used.add(key);
    return true;
  }).join(separator);
  const draft = {
    ...
    cultural: distinctField(data.culturalContext || data.cultural_context || data.cultural || data.culturalBackground || data.historicalOrigin || '', 2, ' / '),
    elements: distinctField(data.designElements || data.elements || data.visibleFacts || '', 6, ' / '),
    palette: [...new Set((Array.isArray(data.palette) ? data.palette : String(data.palette || '').split(/[,，、\s]+/)).map(color => String(color).trim().toUpperCase()).filter(validHex))].slice(0, 6).join(', '),
    tags: cleanGeneratedTags(data.tags || data.styleTags).join(', '),
    composition: distinctField(data.composition || '', 1, ''),
    useCases: distinctField(data.useCases || data.use_cases || '', 5, ', '),
    promptZh: String(data.promptZh || data.prompt_zh || '').trim(),
    promptEn: String(data.promptEn || data.prompt_en || '').trim(),
    negativeZh: data.negativePromptZh || data.negative_prompt_zh || data.negativeZh || data.negative_zh || data.negative || data.negativePrompt || data.negative_prompt || '',
    negativeEn: data.negativePromptEn || data.negative_prompt_en || data.negativeEn || data.negative_en || data.negative || data.negativePrompt || data.negative_prompt || '',
  ...
  const invalidTitle = /^(untitled|aesthetic system|visual reference)$/i.test(draft.title) || /^(未命名|未命名视觉参考|视觉参考)$/.test(draft.titleZh);
  const missingCore = !draft.elements || !draft.composition || !draft.palette || !draft.useCases || draft.promptZh.length < 80 || draft.promptEn.length < 80;
  const mixedPromptLanguage = /[\u4e00-\u9fff]/.test(draft.promptEn) || !/[\u4e00-\u9fff]/.test(draft.promptZh);
  if (invalidTitle || missingCore || mixedPromptLanguage) throw new Error('AI 返回的卡片不符合内容标准，请重新分析；当前结果未写入表单。');
  return draft;
}
...
async function callGatewayVision(imageDataList, settings) {
  if (!settings?.id || !settings?.visionModel || providerSyncState !== 'server') return null;
  const images = imageDataList.slice(0, 6).map((imageData, index) => {
    const match = String(imageData).match(/^data:(image\/(?:jpeg|png|webp));base64,(.+)$/i);
    if (!match) throw new Error(`第 ${index + 1} 张图片格式不受支持，请使用 JPG、PNG 或 WebP`);
  ...
  const response = await fetch('/api/ai/analyze-image', {
    ...
    credentials: 'same-origin',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ providerId: settings.id, model: settings.visionModel, images, templateId: template.id, templateVersion: template.version || 1, projectContext: `使用模板「${template.name}」。分析重点：${template.focus}。生成规则：${template.instructions}` })
  });
  const payload = await response.json().catch(() => ({}));
  if (!response.ok) throw providerError(payload, response.status);
...
async function analyzeImageWithProvider(imageDataList, providerSettings) {
  if (!providerSettings?.hasSecret || !(providerSettings.visionModel || providerSettings.textModel)) return null;
  try {
    const draft = await callGatewayVision(imageDataList, providerSettings);
...
function readImageFile(file) {
  return new Promise(resolve => {
    if (!['image/jpeg', 'image/png', 'image/webp'].includes(file.type)) return resolve('');
...
async function analyzeArchiveImage() {
  ...
  if (!imageList[0]) return toast(document.documentElement.lang === 'en' ? 'Upload at least one JPG, PNG, or WebP image first' : '请先上传至少一张JPG、PNG或WebP图片');
  if (imageList.some(image => !/^data:image\/(?:jpeg|png|webp);base64,/i.test(image))) return toast(isEnglish() ? 'AI analysis currently supports uploaded local images only. URLs remain available for card display.' : 'AI分析目前只支持本地上传图片；图片地址仅用于卡片展示。');
  const settings = selectedProviderSettings();
  const useProvider = providerReady();
  if (!useProvider) {
    if (els.archiveAiStatus) els.archiveAiStatus.textContent = isEnglish() ? 'Configure a vision-capable AI service before analysis.' : '请先配置支持图片识别的AI服务；系统不会生成模拟内容。';
    return toast(isEnglish() ? 'Configure an AI service first' : '请先配置AI服务');
  }
  setGenerationBusy(els.archiveAiBtn, true, { busy: document.documentElement.lang === 'en' ? 'Analyzing…' : '分析生成中…' });
  if (els.archiveAiStatus) els.archiveAiStatus.textContent = isEnglish()
    ? `Analyzing all references with ${settings.name || settings.type} / ${settings.visionModel}...`
    : `正在使用 ${settings.name || settings.type} / ${settings.visionModel} 分析全部参考图...`;
  setArchiveStatus(isEnglish() ? 'Analyzing all reference images...' : '正在分析全部参考图...', 'saving');
  let providerDraft = null;
  try {
    providerDraft = await analyzeImageWithProvider(imageList, settings);
  } catch (error) {
    if (els.archiveAiStatus) els.archiveAiStatus.textContent = `Provider 调用失败：${error.message}`;
  ...
  if (els.archiveAiStatus) els.archiveAiStatus.textContent = isEnglish() ? 'Analysis complete. Review the structured draft before saving.' : '分析完成，请审核结构化草稿后再保存。';
  if (imageList.length > 1 && els.archiveAiStatus) els.archiveAiStatus.textContent += ` 已共同分析 ${Math.min(imageList.length, 6)} 张参考图。`;
  ...
  setArchiveStatus(isEnglish() ? 'Ready. Review every field before saving.' : '已生成草稿，请逐项审核后再保存。', 'success');
...
function bindEvents() {
  ...
  els.archiveFile?.addEventListener('change', event => {
    ...
    Promise.all(files.slice(0, 6).map(file => readImageFile(file))).then(images => {
      ...
      if (els.archiveAiStatus) els.archiveAiStatus.textContent = providerReady()
        ? (isEnglish() ? `${images.length} original image(s) loaded. Analyze all references to generate a structured draft.` : `已载入 ${images.length} 张原图。点击AI分析后将共同分析全部参考图。`)
        : (isEnglish() ? `${images.length} original image(s) loaded. Configure an AI service before analysis.` : `已载入 ${images.length} 张原图。请先配置AI服务；系统不会生成模拟内容。`);
    });
  });
  ...
  els.avatarFile?.addEventListener('change', event => {
    ...
    compressAvatarFile(file).then(value => { if (value) { els.avatar.value = value; renderAvatarPicker(value); } });
  });
